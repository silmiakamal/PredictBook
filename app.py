...
if file_sebelumnya and file_sekarang and kolom and keyword:
    if st.button("Tampilkan jumlah keyword"):
        ...
        data_sebelumnya = df_sebelumnya[df_sebelumnya[kolom].str.contains(keyword, case=False,na=False)]
        # Menghilangkan kolom index
        data_sebelumnya = data_sebelumnya.reset_index(drop=True)
        # Menambahkan kolom baru (nomor urut)
        data_sebelumnya.insert(0, "No.", range(1, len(data_sebelumnya)+1))
        st.write(data_sebelumnya[['No.','Judul']], index=False)
        st.write(f"Jumlah keyword '{keyword}' pada file csv tahun sekarang: {jumlah_sekarang}")
        data_sekarang = df_sekarang[df_sekarang[kolom].str.contains(keyword, case=False,na=False)]
        # Menghilangkan kolom index
        data_sekarang = data_sekarang.reset_index(drop=True)
        # Menambahkan kolom baru (nomor urut)
        data_sekarang.insert(0, "No.", range(1, len(data_sekarang)+1))
        st.write(data_sekarang[['No.','Judul']], index=False)
...
if file_sebelumnya and file_sekarang and kolom and keyword:
    if st.button("Prediksi data untuk tahun berikutnya"):
        ...
        # Menampilkan grafik
        st.pyplot()
        st.write(f"Prediksi Pinjaman buku untuk tahun berikutnya dari keyword: {keyword} adalah {predicted_data[2]}")
        
st.sidebar.subheader("Tentang App")
st.sidebar.info(" Ini adalah aplikasi web untuk memprediksi buku yang dipinjam tahun berikutnya di Perpustakaan dengan Hidden Markov Model .  _bySilmiAfifahKamal_")
st.sidebar.info("Perhatian: Ini hanya prediksi.")